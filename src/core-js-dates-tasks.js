...
function dateToTimestamp(date) {
  const startDate = new Date('01 Jan 1970 00:00:00 UTC');
...
function isDateInPeriod(date, period) {
  const curr = new Date(date).valueOf();
  const end = new Date(period.end).valueOf();
  const start = new Date(period.start).valueOf();
...
function formatDate(date) {
  const formated = new Date(date).toLocaleString('en-US', { timeZone: 'UTC' });
  return formated;
}
...
function getWeekNumberByDate(date) {
  ...
  const dayNum = currDate.getUTCDay() || 7;
  currDate.setUTCDate(currDate.getUTCDate() + 4 - dayNum);

  const yearStart = new Date(Date.UTC(currDate.getUTCFullYear(), 0, 1));

  const weekNumber = Math.ceil(
    ((currDate - yearStart) / (1000 * 60 * 60 * 24) + 1) / 7
  );
...
function getNextFridayThe13th(date) {
  ...
  for (let i = 0; i < 24; i += 1) {
    ...
    if (candidateDate >= startDate && candidateDate.getDay() === 5) {
  ...
  throw new Error('Friday the 13th not found (unexpected)');
}
